...
> Atualização de 13/09/2026: este documento registra a referência visual do protótipo, preservado em `legacy/`. A aplicação principal agora usa Next.js, Tailwind v4 e componentes Radix em `src/components/ui`, com autenticação e dados reais via Supabase. `src/styles/tokens.css` continua como base da identidade; `src/app/globals.css` define a integração Tailwind e o tema claro. Os valores do banco são strings decimais em reais. As descrições abaixo sobre Vite, dados demonstrativos, centavos em memória e ausência de autenticação se referem somente ao protótipo. Consulte `README.md` para o funcionamento e a validação atuais.
